# MoShaabn.CleanArch
## I Added some missing configurations for api building
1. jwt
2. CurrentUser interface for jwt
3. MappingProfiles
4. internal storage
5. docker setup
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p cace9057-954d-473e-a87c-01b77ce95fb7
```

> `cace9057-954d-473e-a87c-01b77ce95fb7` is the password of the certificate, you can change it to any password you want.
...
Run `MoShaabn.CleanArch.DbMigrator` to create the initial database. This should be done in the first run. It is also needed if a new database migration is added to the solution later.

### Solution structure

This is a layered monolith application that consists of the following applications:

* `MoShaabn.CleanArch.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
* `MoShaabn.CleanArch.HttpApi.Host`: ASP.NET Core API application that is used to expose the APIs to the clients.
